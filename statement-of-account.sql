...
,Y.`Txn Type`
,concat(
ifnull(trim(Y.`CheqNo`),' ')
,ifnull(trim(Y.`Description`),' ') ) as 'Descriptions'
,Y.`Value Date`
,Y.`Debit`
,Y.`Credit`
,Y.`Running Total`
...
,(SELECT IFNULL(SUM(sat.amount),0) FROM m_savings_account_transaction sat WHERE sat.savings_account_id = sa.id AND sat.transaction_type_enum in (2,4,5,7,17,19) AND sat.is_reversed = 0 AND sat.transaction_date < ${startDate} ) as 'Debit'
,(SELECT IFNULL(SUM(sat.amount),0) FROM m_savings_account_transaction sat WHERE sat.savings_account_id = sa.id AND sat.transaction_type_enum in (1,3,8,16) AND sat.is_reversed = 0 AND sat.transaction_date < ${startDate}) as 'Credit'
...
,concat(
ifnull(trim(att.description),'') 
,ifnull(concat(' from SvgsAct - ',atd.from_savings_account_id),'') 
,ifnull(concat(' from LoanAct - ',att.from_loan_transaction_id),'')
,ifnull(concat(' to SvgsAct - ',atd.to_savings_account_id),'') 
,ifnull(concat(' to LoanAct - ',atd.to_loan_account_id),'')
,ifnull(ch.name,'')
) as 'Description'
